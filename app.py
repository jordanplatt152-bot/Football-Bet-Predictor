...
def betting_board(data: pd.DataFrame, demo: bool) -> None:
    ...
    if demo:
        ...
        st.info("No markets currently satisfy every model, value, liquidity, freshness and pre-match control.")
    else:
    ...
    statuses = c3.multiselect("Status", sorted(data.dashboard_status.unique()))
    view = data
    if leagues: view = view[view.competition.isin(leagues)]
    if markets: view = view[view.market.isin(markets)]
    if statuses: view = view[view.dashboard_status.isin(statuses)]
    st.dataframe(view[["fixture", "market", "model_grade", "price_grade", "fractional_odds",
                       "effective_exchange_odds", "model_percent_display", "edge_percent_display",
                       "ev_percent_display", "available_liquidity_gbp", "dashboard_status",
                       "dashboard_rejection_reasons"]], use_container_width=True, hide_index=True,
                 column_config={
                     "model_percent_display": st.column_config.NumberColumn("Model %", format="%.1f%%"),
                     "edge_percent_display": st.column_config.NumberColumn("Edge %", format="%.1f%%"),
                     "ev_percent_display": st.column_config.NumberColumn("EV %", format="%.1f%%"),
...
def match_analysis(data: pd.DataFrame) -> None:
    ...
    for _, row in rows.iterrows():
        with st.container(border=True):
            st.markdown(f"### {row.market} · {row.dashboard_status}")
            ...
            if row.dashboard_rejection_reasons:
                st.caption("Controls: " + row.dashboard_rejection_reasons)
...
with st.sidebar:
    ...
    st.caption("V1.2.1 · automatic five-minute safety refresh")
